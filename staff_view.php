...
<div class='alert alert-danger' style="display: none;"><strong>Error !</strong> Employee Id Missing !</div>
		<div class="card">
			<div class="card-header">
				<h1>
					<a class="btn btn-info float-right" href="staffdate_view.php"><h1>Back</h1></a>
				</h1>
			</div>
			<?php 
			$date=$_GET['att_time'];
...
if(isset($_POST["submit"]))
{
 ...
 $qry="update staff_attendance set attend='$atn_value', att_time='$date' where sid=$atn_key";
 $rs=executequery($qry);
 if($rs=="success")
 {
 ...
 <?php
$results_per_page = 6;
$qry="SELECT * FROM staff_attendance where att_time='$date'";
$rs=readrecord($qry);
...
$query = "SELECT * FROM staff_attendance where att_time='$date' LIMIT " . $page_first_result . ',' . $results_per_page;  
...
$i++;
        ...
        <?php
				 ...
				 if($page>=2){   
					echo " <li class='page-item' style='border:none;'> <a  class='page-link' href='staff_view.php?page=".($page-1)."'> <h1> Prev </h1></a></li>";   
				}
				for($i = 1; $i<= $number_of_page; $i++) {  
        echo '<li class="page-item"> <a class="page-link" href = "staff_view.php?page=' . $i . '"><h1>' . $i . ' </h1></a></li>';  
	}
	
	if($page<$number_of_page){ 
  
		echo "<li class='page-item' style='border:none;'><a  class='page-link' style='border:1px;' href='staff_view.php?page=".($page+1)."'> <h1> Next </h1></a></li>";   
	}   
	echo "</ul>"
